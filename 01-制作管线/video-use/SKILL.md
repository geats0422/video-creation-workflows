...
- **Virtual environment activated** — Run `D:\work\OPC\OPC\Scripts\activate` before any skill execution.
...
- **Use wider review padding for rough cuts.** For documentary/scripted rough-cut packages, prefer 200–800ms tail room on complete sentences and scene endings. Do not put the cut exactly on the final recognized word; ASR/decoder drift can swallow word endings.
...
- If the speaker says `OK`, `好`, `keep`, or `good` immediately after a take, keep the preceding take.
- If the speaker says `pass`, `不要`, `cut`, `again`, or `redo` immediately after a take, discard the preceding take and prefer the later retake.
