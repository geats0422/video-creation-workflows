...
Read install.md first to install this repo, wire up ffmpeg, register the skill with whichever agent you're running under. Then read SKILL.md for daily usage, and always read helpers/ because that's where the editing scripts live. After install, don't transcribe anything on your own — just tell me it's ready and wait for me to drop footage into a folder.
```

The agent handles the clone, dependencies, skill registration.
...
Recommended per-video project layout:
...
- Put the script under `video script\` before editing.
- Use the script filename as the video title.
- Specify the episode number as `第X期` when starting a project.
- If the episode number is missing, check existing folders under `D:\work\OPC\videos\` and use the next episode number as a reminder.
- Final folder name format: `第X期：{视频标题}`.

```bash
# 1. Activate virtual environment
D:\work\OPC\OPC\Scripts\activate

# 2. Enter video-use directory
cd D:\work\OPC\video-use

# 3. Transcribe a video
python helpers/transcribe.py <video_path> --language zh

# 4. Generate a Wondershare Filmora project
python helpers/generate_filmora_project.py edit/edl.json -o edit/project.wfpx --name "My Project"
```

Filmora project generation auto-detects the nearest `ProjectFolder` beside `edl.json`, so `D:\work\OPC\videos\{第X期：视频标题}\ProjectFolder` is used automatically when your `edl.json` is inside that project. Fallback template: `D:\work\OPC\video-use\123\ProjectFolder`.
...
# 2. Create virtual environment (Python 3.12 recommended for compatibility)
uv venv OPC --python 3.12

# 3. Install deps
cd ~/Developer/video-use
uv pip install -e . --python ../OPC/Scripts/python.exe
...
**Layer 1 — Audio transcript (always loaded).** Whisper + Fun-ASR transcription per source gives word-level timestamps, speaker diarization, and audio events (`(laughter)`, `(applause)`, `(sigh)`). All takes pack into a single ~12KB `takes_packed.md` — the LLM's primary reading view.
