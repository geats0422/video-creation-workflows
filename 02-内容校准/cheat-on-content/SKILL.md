...
在 `D:\work\OPC\videos\{第X期：视频标题}\` 视频工程内使用时，本 skill 的所有输出必须集中写入该视频工程下的独立工作台：

```text
D:\work\OPC\videos\{第X期：视频标题}\content ops\cheat-on-content\
```

解析 `{第X期：视频标题}` 的优先级：

1. 用户显式给出视频项目文件夹时，使用该文件夹。
2. 输入稿件位于 `D:\work\OPC\videos\...\video script\` 或 `D:\work\OPC\videos\...\video scripts\` 时，使用其父级视频项目文件夹。
3. 输入来自 `Raw Footage\`、`edit\`、`assets\`、`ProjectFolder\` 时，使用这些目录的父级视频项目文件夹。
4. 如果只给出期数和标题，组合为 `D:\work\OPC\videos\{第X期：视频标题}\`。

除非用户明确要求在其他内容项目中运行，否则把下面的 `<user-content-project>` 视为：

```text
D:\work\OPC\videos\{第X期：视频标题}\content ops\cheat-on-content\
```

不要把 `rubric_notes.md`、`predictions\`、`samples\`、`candidates.md`、`.cheat-state.json` 等文件写到视频工程根目录、`Raw Footage\`、`video script\`、`video scripts\`、`edit\`、`assets\` 或 skill 目录。

`Raw Footage\` 是用户拍摄原片目录，只可作为拍摄状态、分镜范围和实际稿件差异的参考，不可改名、移动、覆盖或写入派生文件。
